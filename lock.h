...
{                                                          \
  ...
  if(heldBy != pthread_self()) {                           \
     ...
     LDBG(("LOCK[%u]: aquired\n", pthread_self()));        \
     if(__ptmlret) {                                       \
        printf("libmfperf: pthread_mutex_lock returned %d\n",  \
               __ptmlret);                                 \
        exit(-1);                                          \
     }                                                     \
     heldBy = pthread_self();                              \
...
{                                                           \
  ...
  if(heldBy == pthread_self()) {                            \
     ...
     if(sema == 0) {                                        \
       ...
       if(__ptmulret) {                                     \
          printf("libfperf: pthread_mutex_unlock returned %d\n", \
                 __ptmulret);                               \
          exit(-1);                                         \
       }                                                    \
     } else {                                               \
     ...
     printf("libfperf: mutex_unlock: thread %u doesn't hold the lock. (held by %u)\n", \
            pthread_self(), heldBy);                        \
     return;                                                \
  }                                                         \
}
